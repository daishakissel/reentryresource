...
export default function AdminResourcesPage() {
  ...
  // Selection scoped to currently-visible (filtered) resources
  const selectedVisibleIds = useMemo(
    () => filteredResources.filter((r) => selectedIds.has(r.id)).map((r) => r.id),
    [filteredResources, selectedIds]
  );
  const allVisibleSelected = filteredResources.length > 0 && selectedVisibleIds.length === filteredResources.length;
  ...
  function toggleSelectAll() {
    setSelectedIds((prev) => {
      ...
      if (allVisibleSelected) filteredResources.forEach((r) => next.delete(r.id));
      else filteredResources.forEach((r) => next.add(r.id));
      return next;
    });
  }
  ...
  async function handleBulkDelete() {
    ...
    const results = await Promise.all(
      ids.map((id) =>
        fetch(`/api/admin/resources/${id}`, { method: "DELETE", headers })
          .then((r) => r.ok)
          .catch(() => false)
      )
    );
    ...
    if (failed > 0) alert(`Failed to delete ${failed} resource${failed !== 1 ? "s" : ""}.`);
    fetchResources();
  }
    ...
    <div>
          ...
          <div className="relative">
            ...
            >
              ...
              {selectedVisibleIds.length > 0 && (
                <span className="text-xs px-1.5 py-0.5 rounded-full bg-brand-gold text-white">{selectedVisibleIds.length}</span>
              )}
            ...
            {showActions && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1 z-40 bg-white dark:bg-ocean-dark border border-gray-200 dark:border-ocean-light rounded-lg shadow-lg py-1 w-52">
                  <button
                    disabled={selectedVisibleIds.length === 0}
                    onClick={() => { setShowActions(false); setShowBulkDeleteConfirm(true); }}
                    className="w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 disabled:text-gray-400 disabled:cursor-not-allowed disabled:hover:bg-transparent flex items-center gap-2"
                  >
        ...
        <div className="border border-gray-200 dark:border-ocean-light rounded-lg overflow-x-auto">
          <table className="w-full text-sm whitespace-nowrap">
            <thead className="bg-gray-50 dark:bg-ocean-dark">
              <tr>
                <th className="px-4 py-2 w-10 align-top">
                  <input
                    ref={selectAllRef}
                    type="checkbox"
                    ...
                    className="h-4 w-4 mt-1 rounded border-gray-300 text-brand-gold focus:ring-brand-gold cursor-pointer"
                  />
                </th>
            ...
            <tbody>
              {filteredResources.map((r) => (
                <tr key={r.id} className={`border-t border-gray-100 dark:border-ocean-light hover:bg-gray-50 dark:hover:bg-ocean-light ${selectedIds.has(r.id) ? "bg-brand-gold/5" : ""}`}>
      ...
      {showBulkDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/50" onClick={() => !bulkDeleting && setShowBulkDeleteConfirm(false)} />
          <div className="relative bg-white dark:bg-ocean-dark rounded-lg shadow-xl p-6 w-full max-w-md">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Delete resources?</h2>
            <p className="text-sm text-gray-600 dark:text-gray-300 mb-6">
              Are you sure you want to delete <span className="font-semibold">{selectedVisibleIds.length}</span> resource{selectedVisibleIds.length !== 1 ? "s" : ""}? This cannot be undone.
            </p>
            <div className="flex justify-end gap-2">
              <button
                onClick={() => setShowBulkDeleteConfirm(false)}
                disabled={bulkDeleting}
                className="px-4 py-2 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-ocean-light hover:bg-gray-200 dark:hover:bg-ocean transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              <button
